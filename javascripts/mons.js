$(function () {
    ...
    if (!avid) avid = getURLParameter('id');
    ...
    // 从 URL 参数获取其他参数
    var EG = $('#EG').val() ? parseInt($('#EG').val()) : (getURLParameter('eg') ? parseInt(getURLParameter('eg')) : 1)
    var HLG = $('#HLG').val() ? parseInt($('#HLG').val()) : (getURLParameter('hlg') ? parseInt(getURLParameter('hlg')) : 1)
    var DEF = $('#DEF').val() ? parseInt($('#DEF').val()) : (getURLParameter('def') ? parseInt(getURLParameter('def')) : 1000)
    var LV = $('#LEVEL').val() ? parseInt($('#LEVEL').val()) : (getURLParameter('lv') ? parseInt(getURLParameter('lv')) : 95)
    var MULT = 1
    ...
    // 已加载的技能文件缓存
    var loadedSkillFiles = {};

    // 动态加载怪物技能文件
    function loadMonsterSkillFile(monsterId, callback) {
        if (loadedSkillFiles[monsterId]) {
            if (callback) callback();
            return;
        }

        var filePath = '/sr/data/' + lang3 + '/skills/' + monsterId + '.js';
        var script = document.createElement('script');
        script.src = filePath;
        script.onload = function() {
            loadedSkillFiles[monsterId] = true;
            if (callback) callback();
        };
    ...
    //获取怪物技能 重写 优先从/sr/data/CH/skills/id.js读取然后才是monsterskill大表 byMIAO
    function getMonsterSkill(skid, monsterId) {
        // 如果文件未加载，尝试加载对应文件
        if (!loadedSkillFiles[monsterId]) {
            loadMonsterSkillFile(monsterId);
            // 返回默认对象，因为文件还在加载中
            return { Name: '', SP: 0 };
        }
        // 尝试从对应怪物ID的技能对象中查找
        var monsterSkillObjName = '_monsterskill_' + monsterId;
        console.log('Looking for skill:', skid, 'in', monsterSkillObjName, 'exists:', window[monsterSkillObjName] !== undefined);
        if (window[monsterSkillObjName] && window[monsterSkillObjName][skid]) {
            return window[monsterSkillObjName][skid];
        }
        return [];
    }
    ...
    function renderSkill(cur_mon) {
        ...
        // 渲染变种怪物
        if (cur_mon.variant && cur_mon.variant.length) {
            $('.mon_body_').render({
                div: [
                    {
                        div: (lang == 'CH') ? '变种' : 'Variants',
                        class: 'a_section_head'
                    },
                    {
                        div: [],
                        class: 'a_section_content variant_wrap',
                        style: {
                            display: 'flex',
                            'flex-wrap': 'wrap'
                        }
        ...
        var monsterId = cur_mon._id
        loadMonsterSkillFile(monsterId, function() {
            sk.forEach(function (skid) {
                var skill = getMonsterSkill(skid, monsterId)
    ...
    //渲染变种详细信息
    function render_variant(varid, var_mon) {
    if (!var_mon) return
    $('.variant_wrap').render({
        a: '/sr/monster/#_' + varid,
        t: [
            {
                div: [
                    {
    ...
    function renderStat(cur_mon) {
            ...
            $('.basestat').empty().render([
                {
                    div: [
                        {
                            div: (lang == 'CH') ? '等级选择' : 'Level Selection',
                            style: {
                                'margin-bottom': '10px',
                                'font-weight': 'bold',
                                'color': '#ffffff'
        ...
        if (cur_mon.Multistage && cur_mon.Multistage > 1) {
            var multistage_factor = 1
            ...
            $('.base_stat_multistage').empty().render({
                div: [
                    {
                        span: (lang == 'CH') ? '阶段：' : 'Phase: ',
                        style: {
                            margin: '0 5px 0 0',
                            'align-self': 'center'
                        }
            ...
            // 绑定阶段选择事件
            $('body').off('change', '.phase_select').on('change', '.phase_select', function() {
                // 更新全局的_stage变量
                _stage = parseInt($(this).val().replace('阶段', '')) || 1
                //alert(cur_mon.Multistage_list[_stage-1])
                // 当选择阶段时，重新渲染整个基础属性部分
                renderStat(cm)
            })
        } else {
            // 如果没有Multistage参数或小于等于1，清空base_stat_multistage
            $('.base_stat_multistage').empty()
        }
        //基础数值
    ...
    function updatePhaseData(cur_mon, phase) {
        // 这里填充阶段数据
        // 示例：根据选择的阶段计算并显示数据
        $('.phase_data').empty().render({
            span: '12345' // 这里显示你的数字
        })
    }
    ...
    // 滑条事件处理 - 使用 change 事件而不是 input 事件，保证滑动流畅性
    $('body').on('change', '#level_slider', function () {
        var new_lv = parseInt($(this).val());
        if(new_lv===101) LV=120
        else LV = new_lv
        $('.base_stat_lv').val(LV);
        renderStat(cm);
    })
