...
$(function () {
    ...
    // 初始化_weapondesc和_weaponskill为空对象，避免依赖Avatar.js
    var _weapondesc = {}
    var _weaponskill = {}
    ...
    function renderCL(v) {
        ...
        $('.cl_data').render({
            div: {
                ...
                click: function (l) {
                    download(`/Char/HSR/${change_filename}/${VER_SR}.png`, `${change_filename}`)
                },
                style: {
                    width: 'max-content',
                    padding: '10px 20px',
        ...
        _diff_avatar.forEach(function (i, j) {
            ...
            if (!loaded.includes(avatar_data._id)) {
                let script_computer_a = document.createElement('script')
                script_computer_a.src = '/sr/data/' + lang3 + '/Avatar/' + avatar_data._id + '.js'
                document.head.append(script_computer_a)
                script_computer_a.onload = function () {
            ...
            var tname = avatar_data.Name
            var tcolor = avatar_data.Element
            var ticon = avatar_data.Icon.replaceAll("avatarshopicon", "avataricon").replaceAll("/Avatar/", "/avatar/")
            var shows_1 = _changelog_avatar[i] ? _changelog_avatar[i][vn] : []
            if (!shows_1) shows_1 = []
            var shows = shows_1.concat(d_avatar(avatar_data, vo, vn))
            if (!shows.length) return
            $('.cl_data').render({
                div: [
                    {
    ...
    function d_avatar(av_data, vo, vn) {
        // 检查角色数据是否存在
        if (!av_data || !av_data.Skills) {
            console.error('av_data is undefined or missing Skills:', av_data);
            return []
        }
        var oo = {}
        var nn = {}
        ...
        for (it in skill_list) {
            id = skill_list[it]
            // 检查技能数据是否已加载
            if (!_avatarskill[id] || !_avatarskill[id][vo] || !_avatarskill[id][vn]) {
                console.error('Skill data missing for id:', id, 'vo:', vo, 'vn:', vn);
                continue
            }
        ...
        // Skip skill tree comparison if data is missing for either version
        if (o_t && n_t) {
            if (!o_t.Tree4 || !Object.keys(o_t.Tree4).length) o_t.Tree4 = { Name: '', Desc: '' }
            if (!n_t.Tree4 || !Object.keys(n_t.Tree4).length) n_t.Tree4 = { Name: '', Desc: '' }
        ...
        for (it in av_data.Ranks) {
            id = av_data.Ranks[it]
            var o_rank = _avatarrank[id][vo]
            var n_rank = _avatarrank[id][vn]
            // Skip if rank data is missing for either version
            if (!o_rank || !n_rank) continue
            oo = o_rank.Desc.replaceAll(".0 ", " ").replaceAll(".0</b>", "</b>").replaceAll(".0</color>", "</color>").replaceAll(".0%", "%").replaceAll(".0.", ".").replaceAll(".0,", ",").replaceAll(".0#", "#").replaceAll("<u>", "").replaceAll("</u>", "")
            nn = n_rank.Desc.replaceAll(".0 ", " ").replaceAll(".0</b>", "</b>").replaceAll(".0</color>", "</color>").replaceAll(".0%", "%").replaceAll(".0.", ".").replaceAll(".0,", ",").replaceAll(".0#", "#").replaceAll("<u>", "").replaceAll("</u>", "")
            if (oo != nn) {
                out.push('<b>@' + n_rank.Name + '# E' + (parseInt(it) + 1) + '</b><br>' + dif(oo, nn, 16))
            }
        }
        return out
    }
    ...
    function d_weapon(wpn_data, vo, vn) {
        // 检查武器数据是否已加载
        if (!_weaponskill[wpn_data._id] || !_weaponskill[wpn_data._id][vo] || !_weaponskill[wpn_data._id][vn]) {
            console.error('Weapon skill data missing for id:', wpn_data._id, 'vo:', vo, 'vn:', vn);
            return []
        }
            ...
            template: [
                ...
                },
                {
                    section: [
                        ...
                        {
                            schedule: {
                                img: '/images/Paths/Elation.png'
                            },
                            a: {
                                'data-id': 'Elation'
                            },
                            class: global_var_type.Elation ? 'active' : ''
                        },
                    ],
    ...
    function popAvatar(ai) {
        ...
        if (!loaded.includes(this_avatar._id)) {
            let script_computer_a = document.createElement('script')
            script_computer_a.src = '/sr/data/' + lang3 + '/Avatar/' + this_avatar._id + '.js'
            document.head.append(script_computer_a)
            script_computer_a.onload = function () {
    ...
    function renderAvatarAfter(i) {
        ...
        if (i == 3 || i == 10) {
            ...
            skill_list.forEach(function (s, i) {
                ...
                if (S.MaxLevel == 9) recommendLV = 6
                if (S.MaxLevel == 10) recommendLV = 6
                // 确保recommendLV不超出Params数组范围
                if (S.Params && S.Params.length > 0) {
                    recommendLV = Math.min(recommendLV, S.Params.length)
                }
                if (S.Stance) {
                    var stance_0 = (S.Stance[0] * 10)
                    var stance_1 = (S.Stance[1] * 10)
                    ...
                    div: [
                        ...
                        },
                        {
                            div: [
                                ...
                                {
                                    p: param(S.Desc, S.Params && S.Params.length > 0 ? S.Params[Math.min(recommendLV - 1, S.Params.length - 1)] : []),
                                    class: 'sd sd-' + s
                                },
                                {
                                    div: function (b) {
        ...
        if (i == 2 || i == 10 || i == 8 || i == 3) {
            var fu = setInterval(function () {
                if (loaded.includes(this_avatar._id)) {
                    clearInterval(fu); // 清除setInterval，避免无限循环
                    if (i == 2 || i == 10 || i == 3) {
                        $('.gacha').render({
                        ...
                        $('.as_left').remove()
                        setTimeout(function () { initSpineGacha(this_avatar._id) }, 300)
                    }
                    var mats_list = [
                        {
                            div: [
                            ...
                            div: [
                                ...
                                },
                                {
                                    ...
                                    style: {
                                        ...
                                        border: '2px solid #f29e38',
                                        'border-radius': '5px',
                                        display: check_pic_dl() ? 'inline-block' : 'none'
                                    },
                                    class: 'hover-shadow pic_dl',
                                    click: function () {
                                        download(`/Char/HSR/${this_avatar.Name}/${VER_SR}.png`, `${this_avatar.Name}`)
                                    }
                                },
                                {
                                    span: [],
                                    class: 'gacha-toggle-wrap'
                                }
                            ],
                            style: {
                                width: '100%',
    ...
    function popWeapon(ai) {
        ...
        rotate()
        $('.content_2').show()
        window.location.hash = '_' + weaponId
        $('h3 .lang').html(`<a class='translate_' href='?lang=CH${window.location.hash}'><b>中文</b></a> | <a class='translate_' href='?lang=EN${window.location.hash}'><b>English</b></a> | <a class='translate_' href='?lang=JP${window.location.hash}'><b>日本語</b></a> | <a class='translate_' href='?lang=KR${window.location.hash}'><b>한국인</b></a>`)
        if (!loaded.includes(weaponId)) {
            let script_computer_a = document.createElement('script')
            script_computer_a.src = '/sr/data/' + lang3 + '/Weapon/' + weaponId + '.js'
            document.head.append(script_computer_a)
            script_computer_a.onload = function () {
                loaded.push(weaponId)
                _weapondesc = {..._weapondesc, ..._weapondesc_}
                _weaponskill = {..._weaponskill, ..._weaponskill_}
                decorateWeapon()
            }
    ...
    function decorateWeapon() {
        this_weapon_vers = getVer(_weaponskill[this_weapon._id])
        ...
        $('.t_skill').render([
            {
                div: [
                    {
                        span: _weaponskill[this_weapon._id][this_weapon_cur_ver].Name,
                        style: {
                            'margin': '0'
                        },
            ...
            },
            {
                div: {
                    ...
                    data: _weaponskill[this_weapon._id][this_weapon_cur_ver].Desc
                },
    ...
    function popRelic(ai) {
        ...
        if (!loaded.includes(this_relic._id)) {
            let script_computer_a = document.createElement('script')
            script_computer_a.src = '/sr/data/' + lang3 + '/Relic/' + this_relic._id + '.js'
            document.head.append(script_computer_a)
            script_computer_a.onload = function () {
    ...
    function param(text, params) {
        ...
        // 安全获取参数值
        function getParam(index, isPercentage = false) {
            const value = params[index];
            if (value === undefined) return "";
            return isPercentage ? (rn(value * 100) + "%") : value;
        }
        
        var PP = text
            .replaceAll("#1[f]", getParam(0))
            .replaceAll("#2[f]", getParam(1))
            .replaceAll("#3[f]", getParam(2))
            .replaceAll("#4[f]", getParam(3))
            .replaceAll("#5[f]", getParam(4))
            .replaceAll("#6[f]", getParam(5))
            .replaceAll("#7[f]", getParam(6))
            .replaceAll("#8[f]", getParam(7))
            .replaceAll("#9[f]", getParam(8))
            .replaceAll("#10[f]", getParam(9))
            .replaceAll("#11[f]", getParam(10))
            .replaceAll("#12[f]", getParam(11))
            .replaceAll("#1[p]", getParam(0, true))
            .replaceAll("#2[p]", getParam(1, true))
            .replaceAll("#3[p]", getParam(2, true))
            .replaceAll("#4[p]", getParam(3, true))
            .replaceAll("#5[p]", getParam(4, true))
            .replaceAll("#6[p]", getParam(5, true))
            .replaceAll("#7[p]", getParam(6, true))
            .replaceAll("#8[p]", getParam(7, true))
            .replaceAll("#9[p]", getParam(8, true))
            .replaceAll("#10[p]", getParam(9, true))
            .replaceAll("#11[p]", getParam(10, true))
            .replaceAll("#12[p]", getParam(11, true));
    ...
    $('body').on('change', '.stat_ver_choose_w select', function () {
        ...
        $('.weapon_skill_name').html(_weaponskill[this_weapon._id][this_weapon_cur_ver].Name)
    ...
    function download(link, name) {
        ...
        var isChanges = link.includes('改动汇总') || link.includes('Changes Compilation')
        var isSkill   = link.includes('/Char/HSR/') && !isChanges

        if (isChanges || isSkill) {
            var target = isChanges ? document.querySelector('.cl_data') : document.querySelector('.mon_body')
            if (!target) { download_cd = 0; return }
            ...
            toast.textContent = isChanges ? '生成中...' : '生成技能长图...'
            document.body.appendChild(toast)

            var opts = { backgroundColor: '#1a1a2e', scale: 2, useCORS: true, logging: false }
            // 改动汇总页用浅色背景
            if (isChanges) opts.backgroundColor = '#1a1a2e'
    ...
    var _spineLibsLoading = false

    function initSpineGacha(avid) {
        var gacha2 = $('.gacha_2')
        if (!gacha2.length) return
        if (gacha2.find('.gacha-spine-wrap').length) return

        fetch('/spine/sr/' + avid + '/manifest.json')
            .then(function (r) { return r.json() })
            .then(function (manifest) {
                var skelPath = '/spine/sr/' + avid + '/' + manifest.skel
                var atlasPath = '/spine/sr/' + avid + '/' + manifest.atlas
                return loadSpineLibs().then(function () {
                    buildSpineToggle(gacha2, skelPath, atlasPath)
                })
            })
            .catch(function () { /* no manifest, skip */ })
    }

    function loadSpineLibs() {
        if (_spineLibsLoading) return _spineLibsLoading
        if (typeof PIXI !== 'undefined' && typeof spine !== 'undefined') {
            _spineLibsLoading = Promise.resolve()
            return _spineLibsLoading
        }
        _spineLibsLoading = new Promise(function (resolve, reject) {
            // 先加载 pixi.js
            if (typeof PIXI !== 'undefined') {
                ensureRequire().then(resolve).catch(reject)
            } else {
                var s1 = document.createElement('script')
                s1.src = '/plugins/pixi.js'
                s1.onload = function () {
                    ensureRequire().then(resolve).catch(reject)
                }
                s1.onerror = reject
            ...
            function ensureRequire() {
                // spine-pixi-v8 用 require("pixi.js") 获取 PIXI 引用，需要补一个 require
                if (typeof require === 'undefined') {
                    window.require = function (name) {
                        if (name === 'pixi.js') return PIXI
                        throw new Error('[Spine] Unknown module: ' + name)
                    }
                }
                if (typeof spine !== 'undefined') return Promise.resolve()
                return new Promise(function (res, rej) {
                    var s2 = document.createElement('script')
                    s2.src = '/plugins/spine-pixi-v8.js'
                    s2.onload = res
                    s2.onerror = rej
    ...
    function buildSpineToggle(gachaDiv, skelPath, atlasPath) {
        var img = gachaDiv.find('img')
        if (!img.length) return

        img.wrap('<div class="gacha-img-wrap"></div>')
        var imgWrap = gachaDiv.find('.gacha-img-wrap')

        var spineWrap = $('<div class="gacha-spine-wrap" style="display:none; position:relative; width:100%; overflow:hidden; contain:layout style paint;"></div>')
        gachaDiv.append(spineWrap)

        var toggleBtn = $('<span class="hover-shadow gacha-toggle-btn" style="display:inline-block; padding:5px 10px; border:2px solid #f29e38; border-radius:5px; font-size:14px; cursor:pointer; user-select:none;">Live2D</span>')
        var toggleWrap = gachaDiv.parent().find('.gacha-toggle-wrap')
        if (toggleWrap.length) {
            toggleWrap.append(toggleBtn)
        } else {
            gachaDiv.append(toggleBtn)
        }

        var showing = 'img'
        var spineReady = false
        var assetsPreloaded = null
        var spineApp = null
        var spineObj = null
        var spineVp = null
        ...
        // 后台预加载资产，避免点击时才加载导致卡顿
        // 直接用URL加载，避免别名冲突导致切换角色时缓存污染
        assetsPreloaded = loadSpineLibs().then(function () {
        ...
        function stageRatio() {
            if (!spineApp || !spineApp.canvas) return 1
            var rect = spineApp.canvas.getBoundingClientRect()
            return rect.width > 0 ? spineApp.screen.width / rect.width : 1
        }

        function applyVp() {
            if (!spineVp) return
            spineVp.scale.set(vpScale)
            spineVp.x = vpX
            spineVp.y = vpY
        }
        ...
        function toggle() {
            if (showing === 'img') {
                ...
                toggleBtn.text('立绘')
                showing = 'spine'
                if (!spineReady) initSpineCanvas()
            } else {
                imgWrap.show()
                spineWrap.hide()
                toggleBtn.text('Live2D')
                showing = 'img'
            }
        }

        toggleBtn.on('click', function (e) { e.stopPropagation(); toggle() })

        function initSpineCanvas() {
            W = gachaDiv.width() || 400
            spineWrap.css({ width: W + 'px', height: W + 'px' })

            var app = new PIXI.Application()
            spineApp = app

            app.init({
                resizeTo: spineWrap[0],
                backgroundAlpha: 0,
                antialias: true,
                autoDensity: true,
                ...
                spineWrap[0].appendChild(app.canvas)
                ...
                obj.x = 0
                obj.y = 0
                obj.scale.set(0.275 * app.screen.width / 1024)
                spineVp.addChild(obj)

                vpScale = 0.5
                vpX = app.screen.width / 2
                vpY = app.screen.height
                applyVp()
                ...
                app.canvas.addEventListener('wheel', function (e) {
                    ...
                    var my = (e.clientY - rect.top) * ratio
                    vpX = mx - (mx - vpX) * (vpScale / old)
                    vpY = my - (my - vpY) * (vpScale / old)
                    applyVp()
                }, { passive: false })
                ...
                window.addEventListener('mousemove', function (e) {
                    if (!dragging) return
                    var ratio = stageRatio()
                    vpX = dvX + (e.clientX - dsX) * ratio
                    vpY = dvY + (e.clientY - dsY) * ratio
                    applyVp()
                })
